...
The soap example, a room of engineer failed to acheieve. Engineering is created when the enivornment are set at constant, which is impossible in a real world. Stuff that cause enviornment change can be temperture, or chemical reactions. These which are all unpredictable and uncontrollable. Engineer will not be able to solve this problem easily. Iso-morphic is when it's easy to create something that look alike and achieve what one want it to look like, but not the concept and funcationality behind it.
